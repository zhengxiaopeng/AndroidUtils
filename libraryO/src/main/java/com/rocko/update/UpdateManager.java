...
public class UpdateManager {
     ...
     *
     * @param context
     * @param isShowProgress 是否显示进度条和检查失败后Toast提示
     */
    ...
    private static boolean isNeedUpdate(Context context) {
        ...
        String currentVersion = ApplicationUtils.getAppVersionName(context);
    ...
    private static UpdateInfo getUpdateInfo(Context context) {
        ...
        try {
            info = updateInfoService.getUpdateInfo(context);
        } catch (Exception e) {
            DebugLog.e(e.getMessage());
            e.printStackTrace();
        }
    ...
    private static class MySimpleDialogConfirmListener implements SimpleDialogConfirmListener {
        @Override
        public void confirm() {
            if (!HttpUtil.isWifiDataEnable(mContext) && !HttpUtil.isMobileDataEnable(mContext)) {
